...
/// <summary>
/// Utilities related to processing <see
/// href="https://docs.sixlabors.com/articles/imagesharp/index.html">ImageSharp</see><see
/// cref="Image"/> s.
/// </summary>
public static class ImageUtils
{
    ...
    public static Image Merge(this IEnumerable<Image> images, MergeDirection direction = MergeDirection.LeftRight, float overlap = 0.25f)
    {
        ...
        foreach (Image img in direction.IsReverse() ? images.Reverse() : images)
        {
            if (direction.IsReverse())
                currentTopOrLeftEdge -= (int)(edgeDimension(img) * (first ? 1 : (1 - overlap)));
            ...
            if (!direction.IsReverse())
                currentTopOrLeftEdge += (int)(edgeDimension(img) * (1 - overlap));
            first = false;
        }
    ...
    /// </param>
    /// <param name="heights">
    /// The <see cref="Image.Height">height</see> s of the images which will be merged.
    /// </param>
    /// <param name="direction">
    /// The <see cref="MergeDirection">direction</see> in which the images will be merged.
    /// </param>
    /// <returns>
    /// An <see cref="Image"/>&lt; <see cref="Rgba32"/>&gt; with dimensions which fit a merged
    /// image, as described above.
    /// </returns>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown only if you pass an <see langword="int"/> outside the valid values of <see
    /// cref="MergeDirection"/> cast to a <see cref="MergeDirection"/>, so don't do that.
    /// </exception>
    ...
    /// <returns>
    /// <see langword="true"/> if the merge direction is either <see
    /// cref="MergeDirection.LeftRight"/> or <see cref="MergeDirection.RightLeft"/>, or <see
    ...
    /// <returns>
    /// <see langword="true"/> if the merge direction must be processed in reverse as described
    /// above, or <see langword="false"/> otherwise.
    /// </returns>
    ...
    /// <returns>
    /// An <see cref="Image"/>&lt; <see cref="Rgba32"/>&gt; corresponding to the original
    /// <c>image</c>, but with the specified points deleted.
    /// </returns>
    public static Image Mask(this Image image, IEnumerable<(int x, int y)> points)
    {
        Image<Rgba32> rgbImage = image.CloneAs<Rgba32>();
        foreach ((int x, int y) in points)
            rgbImage[x, y] = Color.Transparent;
        return rgbImage;
    }
    /// <summary>
    ...
    public static Image Autocrop(this Image image, AutocropType type = AutocropType.Both)
    {
        ...
        {
            foreach (bool b in rowEmptiness.TakeWhile(x => x == true))
                minY++;
            foreach (bool b in (rowEmptiness as IEnumerable<bool>).Reverse().TakeWhile(x => x == true))
                maxY--;
        }
