...
namespace d9.grp
{
    ...
    internal class TsvDocument
    {
        ...
        /// <param name="data">A set of unparsed strings to be converted into <see cref="TsvRow"/>s in this table. If not specified, the table will be empty at first.</param>
        public TsvDocument(ColumnInfoSet columns, IEnumerable<string>? data = null)
        {
            Columns = columns;
            if(data is not null) foreach (string s in data) Add(new TsvRow(columns, s));
        }
        /// <summary>
        /// Gets the <see cref="TsvRow"/> in this table with the specified value in its <see cref="ColumnType.Key">Key</see> column, if any such row exists.
        /// </summary>
        ...
        /// <exception cref="Exception">Thrown if the row's columns do not match those of this table.</exception>
        ...
        /// <exception cref="Exception">thrown if any row's columns do not match those of this table.</exception>
        ...
        public IEnumerable<string> Readable
        {
            get
            {
                ...
                foreach (TsvRow line in Data.OrderBy(x => x.Key).Select(x => x.Value)) yield return line[ColumnNames].InColumns(ColumnWidths);
            }
        }
    }    
}
