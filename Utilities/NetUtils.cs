...
public static class NetUtils
{
    /// <summary>
    /// The <see
    /// href="https://learn.microsoft.com/en-us/dotnet/api/system.net.http.httpclient?view=net-8.0">HttpClient</see>
    ...
    /// folder</see> and prints whether or not it was successful, as well as the response code.
    /// </summary>
    ...
    public static async Task<string?> Download(string url, string? fileName = null)
    {
        ...
        try
        {
            ...
            if (contentLength > 1e6)
                throw new Exception($"File at {url} was of size {contentLength}, which is implausibly large!");
            string? mediaType = response.Content.Headers.ContentType?.MediaType;
            if (mediaType != "image/png")
                throw new Exception($"File at {url} was of type {mediaType}, not image/png!");
    ...
    /// <returns>
    /// The <see cref="Image"/> downloaded, if successful, or <see langword="null"/> otherwise.
    /// </returns>
