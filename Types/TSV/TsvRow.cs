...
public class TsvRow
{
    ...
    /// </summary>
    /// <remarks>
    /// Should never have the default value <c>no key found</c> because of validation in the
    /// constructor. A default value is specified only so the compiler doesn't complain about nullability.
    /// </remarks>
    ...
    public TsvRow(ColumnInfoSet columns, params (string k, string v)[] entries)
    {
        ...
        foreach ((string k, string v) in entries)
        {
            ...
            {
                if (columns.IsKey(k))
                    throw new Exception($"Column {k} is the key column, but its value is null!");
                if (!columns.MayBeNull(k))
                    throw new Exception($"Column {k} is not marked as MayBeNull, but has a null value!");
            }
